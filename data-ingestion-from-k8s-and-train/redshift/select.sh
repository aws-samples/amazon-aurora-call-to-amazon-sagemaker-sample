...
do
  ...
  echo "psql select orders_sorted  exit code="$?
  psql -A -e -t -c "select count(*),text_notnull_1 from orders where updated_at > getdate() - '30 hour'::INTERVAL and updated_at < getdate() - '9.9995 hour'::INTERVAL group by text_notnull_1;"
