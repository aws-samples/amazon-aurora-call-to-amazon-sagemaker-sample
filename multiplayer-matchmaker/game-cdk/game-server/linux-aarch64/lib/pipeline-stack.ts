...
export class StkPipelineStack extends Stack {
  ...
  const buildproject = new codebuild.Project(this, `STKgameServerDockerBuild`, {
      ...
      buildSpec: codebuild.BuildSpec.fromObject({
        ...
        phases: {
          build: {
            commands: [`aws ecr get-login-password --region ${this.region} | docker login --username AWS --password-stdin ${this.account}.dkr.ecr.${this.region}.amazonaws.com/stkbase:latest`,`docker build -t ${this.account}.dkr.ecr.${this.region}.amazonaws.com/${registry.repositoryName}:${baseImageVersion.valueAsString} .`,
