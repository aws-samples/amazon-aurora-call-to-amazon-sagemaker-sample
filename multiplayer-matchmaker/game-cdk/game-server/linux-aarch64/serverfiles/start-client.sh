...
game_theme_track=`psql -A -q -t -w -c "/*start-client.sh*/select theme from trackmap where track='$game_track';"`
echo export TRACKTHEME=$game_theme_track >> /root/.bashrc
	
endpoint=`psql -A -q -t -w -c "/*start-client.sh*/select endpoint from servers where is_ready=1 and substr(location,1,7)=substr('"$game_location"',1,7) and num_active_session<"$MAXPLAYERS" and tracktheme='$game_theme_track' order by created_at desc limit 1;"`
if [ -z ${endpoint}];
then
...
session_id=`psql -A -q -t -w -c "/*start-client.sh*/insert into sessions(client_id,created_at,updated_at,location,endpoint,mode,track,tracktheme,max_players,difficulty,is_active) values ('$POD_NAME',NOW(),NOW(),'$game_location','$endpoint','$game_mode','$game_track','$game_theme_track','$game_max_players','$game_difficulty',1) returning id;"`
if [ -z "$session_id" ]
then
...
if [[ $endpoint != 'lobby' ]]
then
  server_session_id=`psql -A -q -t -w -c "/*start-client.sh*/insert into server_sessions(created_at,updated_at,endpoint,s_location,s_track,s_tracktheme,s_mode,s_difficulty,p_difficulty,p_location,p_track,p_tracktheme,p_mode) select NOW(),NOW(),endpoint,location,track,tracktheme,mode,difficulty,'$game_difficulty','$game_location','$game_track','$game_theme_track','$game_mode' from servers where endpoint='$endpoint' returning id;"`
