...
do
  ...
  echo "psql select exit code="$?
  psql -A -e -t -c "select count(*),text_notnull_1 from orders_sorted where updated_at > getdate() - '30 hour'::INTERVAL and updated_at < getdate() - '9.9995 hour'::INTERVAL and text_notnull_1 like '%3229e5d083f7%' group by text_notnull_1;"
